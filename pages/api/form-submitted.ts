...
export default async function handler(
  ...
  if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(applicant)) {
    ...
    /*
       ...
       보내는 쪽의 감정을 말하지 않는다. "고맙습니다", "마음이 무겁습니다"
       같은 문장은 쓴 사람은 진심이어도 받는 쪽에는 과하게 읽힌다. 무게는
       감정이 아니라 사실에서 나온다 — 몇 기수째인지, 어떻게 뽑는지.

       "서류를 모두 읽습니다" 는 지킬 수 있는 약속이고, 지원자가 가장 알고
       싶어 하는 것이기도 하다. 이 한 줄이 어떤 미사여구보다 낫다.
    */
    const html = shell({
      title: `${gen}기 지원서가 접수되었습니다`,
      lead: `${who ? `${esc(who)}님, ` : ""}지원서가 정상적으로 접수되었습니다.<br><br>NEXT는 ${gen - 1}기수 동안 같은 방식으로 학회원을 선발해 왔습니다. 접수된 서류는 모두 읽고, 면접에서 직접 만나 결정합니다. ${gen}기도 다르지 않습니다.<br><br>이후 일정은 아래와 같습니다.`,
      middle: schedule,
