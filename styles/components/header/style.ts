 ...
 * 학회원 영역 진입점.
 *
 * 다른 항목과 성격이 다르다 — 나머지는 누구나 보는 소개 페이지고 이것만
 * 로그인이 필요하다. 글자 하나를 더 얹으면 여섯 번째 메뉴처럼 읽히므로
 * 아이콘으로 구분한다. 밑줄은 글자 폭 기준의 연출이라 여기선 쓰지 않는다.
 */
export const NavIcon = styled.button<{ isWhite: boolean; selected: boolean }>`
  display: inline-flex;
  align-items: center;
  ...
  background: none;
  cursor: pointer;
  color: ${({ isWhite, selected }) =>
      selected ? THEME.ORANGE : isWhite ? THEME.WHITE : THEME.BLACK};
  transition: color 0.24s cubic-bezier(0.22, 1, 0.36, 1);
